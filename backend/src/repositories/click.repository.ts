...
export async function getAnalyticsByUrlId(urlId: string): Promise<ClickAnalytics> {
  ...
  type TimeRow = { date: string; count: number };
  return {
    total,
    today,
    thisWeek,
    thisMonth,
    byBrowser: byBrowser as CountRow[],
    byOs: byOs as CountRow[],
    byDevice: byDevice as CountRow[],
    byCountry: byCountry as CountRow[],
    byReferrer: byReferrer as CountRow[],
    clicksOverTime: clicksOverTime as TimeRow[],
    recent,
  };
}
